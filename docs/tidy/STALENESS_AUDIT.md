...
| --- | --- | --- | --- | --- | --- | --- |
| High | `docs/WORKFLOW.md` Roles | Current text says Claude Code architects/scopes/plans/validates and writes prompts. The new model says Claude Code is the future-plan architect and Codex-ready implementation explainer, not the final validator. | `docs/WORKFLOW.md`, `AGENTS.md` | Rewrote the role model so Claude Code is the future-plan architect / implementation explainer, Codex runs automated validation and reports evidence, ChatGPT audits, and the user/controller owns manual validation and closeout. | Resolved in 3.6.3 | none |
| High | `AGENTS.md` Roles and Implementation Gate | Current text says Claude Code scopes/plans/validates and writes Codex prompts; it also says Codex does not run validation. The new model needs Codex as implementation + validation-evidence agent and user/controller as manual-validation/closeout authority. | `AGENTS.md`, `docs/WORKFLOW.md`, `docs/CODEX_RULES.md` | Rewrote the role split so Codex owns implementation plus automated-validation evidence and the user/controller owns manual validation and closeout. | Resolved in 3.6.3 | none |
| High | `docs/CODEX_RULES.md` Validation Boundary | Current boundary states validation is user/controller-run and Codex must not execute validation. The new role model needs a deliberate rewrite that says which validations Codex may run or provide, which remain controller-run, and how evidence is reported. | `docs/CODEX_RULES.md`, `docs/WORKFLOW.md` | Rewrote the validation boundary to make automated validation Codex-run, keep manual product validation and closeout user/controller-run, and require raw evidence reporting. | Resolved in 3.6.3 | none |
| Medium | `.claude/skills/tidy-codex-prompt-builder/SKILL.md` | Skill still prohibits telling Codex to run validation and bakes in current branch/open/validation sequencing. It should be realigned only after authoritative workflow docs are updated. | `.claude/skills/tidy-codex-prompt-builder/SKILL.md` | Realigned the prompt-builder skill after the owner workflow docs changed so Codex prompts require automated validation evidence while still forbidding git, version, and closeout-script commands. | Resolved in 3.6.3 | none |
| Medium | `.claude/skills/tidy-validation-judge/SKILL.md` | Skill assumes user-pasted validation/status evidence and says it never runs validation. Reconcile after the Codex validation-evidence boundary is rewritten. | `.claude/skills/tidy-validation-judge/SKILL.md` | Reconciled the validation-judge skill so Codex-reported automated output can count as validation evidence alongside user/controller pasted evidence. | Resolved in 3.6.3 | none |
...
| Medium | `scripts/export-chatgpt-review-context.ps1` | The ChatGPT evidence-packet script still prints stale role/phase copy, including "ChatGPT Architect Mode" and an old `1.4.0` scoping instruction, while the current model is ChatGPT reviewer/stewardship support. | `scripts/export-chatgpt-review-context.ps1`, `docs/WORKFLOW.md` | Realigned the support packet copy with the 3.6.3 role model, including ChatGPT reviewer/auditor wording, Codex automated-validation evidence, user/controller manual approval, and generic next-phase scoping. | Resolved in 3.6.3 | none |
