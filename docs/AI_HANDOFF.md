...
**Current Version**: 1.9.22-alpha - read `STATE.json` for the machine-readable oracle.
...
- The acknowledge-only and one-request-per-operation risks are discharged by 1.9.22. Remaining sync risks are lifecycle scheduling, retry/backoff, concurrent-flush suppression, and stranded `syncing`/`failed` recovery (1.9.26); dashboard component migration remains 1.9.23-1.9.25.
