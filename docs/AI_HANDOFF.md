...
- Outbox replay helpers exist but are not generally connected to runtime dashboard mutations. 1.9.5 wires only create-list capture behind the off-by-default `NEXT_PUBLIC_OFFLINE_WRITE_PROTOTYPE_ENABLED` gate; no list-item capture, replay worker, durable persistence, or dashboard replay is wired yet. The exception is the create-list slice's runtime local-first Dexie read, landed in 1.9.16-1.9.17 inside `lib/dashboard-cache.ts` and enabled by default (see the Optimistic updates section).
