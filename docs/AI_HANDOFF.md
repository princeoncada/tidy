...
- 1.9.16 adds a dev-gated local-first read for the create-list slice: the new gated bridge `lib/sync/local-first-create-list.ts` (flag `NEXT_PUBLIC_LOCAL_FIRST_CREATE_LIST_ENABLED`, default OFF, `isLocalFirstCreateListEnabled`) writes the created list through to Dexie (`db.lists`) and returns a read-back view that `ListAdder.onSuccess` uses as the reconcile payload when the flag is on; flag-off reconciles from the server payload exactly as before. The bridge is fire-and-forget safe (returns null on any failure, falling back to the server payload) and is imported only by `components/list/ListAdder.tsx`. `lib/dashboard-cache.ts` and `trpc/client.tsx` are deliberately NOT touched, so the `local-db-role-audit` guard stays green; moving the read into `dashboard-cache.ts`, enabling by default, and flipping the guard are deferred to 1.9.17.
