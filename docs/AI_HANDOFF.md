...
- `drag-drop.spec.ts` also had test-side timing races: mouse-up could beat dnd-kit's scheduled final pointer-move processing, placement assertions could run before removal of the `data-dnd-dragging="true"` feedback clone and hidden/inert `data-dnd-placeholder`, and `waitForSyncBatch` could exceed the default 30-second test timeout under parallel load. For list-item moves, `dragByMouse` now waits for the destination list's active-target highlight before release, then awaits drag finalization; affected movement tests use a 60-second timeout.
