...
- The next product work is the offline app-shell service worker (1.9.19), then the Dexie read fallback and reconciliation (1.9.20-1.9.21) and the per-slice local-first writes (1.9.22+), per `docs/FUTURE_PLANS.md`; authenticated E2E requires real Supabase credentials and per-worker storage state.
