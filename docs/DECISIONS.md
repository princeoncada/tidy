...
**Decision - keep `seriesComplete = false`; the remaining product work is a new 2.0 arc.** True local-first render and collaboration is a distinct architecture, not an extension of the 1.9.x overlay / outbox-render model. `STATE.json.seriesComplete` stays false and flips only when the 2.0 arc completes (2.0.8).
