...
In-Alpha Correction Contract. An in-alpha fix response (a failed-validation fix, or any fix while STATE.json state = alpha) is shaped in this exact order: (1) a commit-before-fix PowerShell block committing all currently-uncommitted working-tree changes, one `.\scripts\commit.ps1 -Files "<single file>"` command per file, strictly one file per commit (coupled files included), emitted even when validation is red; (2) the debugging-attempt classification; (3) the Section 1 fix master prompt; (4) the Section 2 re-validation block. This repeats before every successive in-alpha fix: prior work is committed before each fix moves forward. "No commit commands before the in-alpha block" means the block is MISSING, never that it is prohibited. This is distinct from the Script-printed command rule (which bars re-typing commands a script prints); the commit-before-fix block is hand-authored from the changed files. Source of truth: docs/WORKFLOW.md (Commit-before-fix).
